...
#   $fpath
# )
compinit -d "${XDG_CACHE_HOME:-${TMPDIR:-/tmp}}/.zcompdump-$$"
...
command -v fd &>/dev/null && eval "$(fd --gen-completions zsh)"
# eval "$(rg --generate=complete-zsh)"
command -v oh-my-posh &>/dev/null && eval "$(oh-my-posh init zsh --config "$devenv_tools_dir/config/xyz.omp.json")"
