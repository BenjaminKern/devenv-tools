...
if [[ "$(uname)" == "Darwin" ]]; then
  DESTDIR="$(realpath "$1")"
  curl -sL https://github.com/BenjaminKern/devenv-tools/releases/download/v0.0.2/devenv-tools-aarch64-macos.tar.xz | tar xfJ - --strip=1 -C "$DESTDIR"
  ...
  curl -sL https://github.com/BenjaminKern/devenv-tools/releases/download/v0.0.2/devenv-tools-x86_64-linux.tar.xz | tar xfJ - --strip=1 -C "$DESTDIR"
