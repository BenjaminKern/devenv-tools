...
TASKS=$(($(nproc) + 1))
CC=("${TARGETDIR}/bin/musl-gcc")
_CFLAGS=( -Os -ffunction-sections -fdata-sections -static )
...
_iconv() {
        if [[ ! -e libiconv-${LIBICONV_VERSION}.tar.gz ]]; then
                curl -LO https://ftp.gnu.org/pub/gnu/libiconv/libiconv-$LIBICONV_VERSION.tar.gz
        fi
        tar zxf libiconv-$LIBICONV_VERSION.tar.gz --skip-old-files
        pushd .
...
_tmux() {
        ...
        _cflags=("${_CFLAGS[@]}" -flto "-I${TARGETDIR}/include/ncursesw/")
