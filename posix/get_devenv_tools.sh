...
echo "Downloading typos..."
APPLICATION_NAME=typos-v$TYPOS_VERSION-x86_64-unknown-linux-musl
if [[ $IS_OSX -eq 1 ]]; then
  APPLICATION_NAME=typos-v$TYPOS_VERSION-aarch64-apple-darwin
fi
curl -sL "https://github.com/crate-ci/typos/releases/download/v$TYPOS_VERSION/$APPLICATION_NAME.tar.gz" | tar xfz - -C $DESTDIR/bin
